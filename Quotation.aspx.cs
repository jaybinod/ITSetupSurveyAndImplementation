...
public partial class Quotation : System.Web.UI.Page
{
    ...
    {
        SqlDataAdapter cmd = new SqlDataAdapter("Select SCIH.*, CM.*, (Select EmployeeName from EmployeeUserMaster where UserID=CM.EmployeeID) SalesPerson from ClientMaster CM, SurveyClientItemHead SCIH where CM.ClientID=SCIH.ClientID and SCIH.quotationdone=0 and SCIH.SubmittedForQuotation=1", conn);
        if (conn.State == ConnectionState.Closed)
            conn.Open();
