...
public partial class HomeSurveyPendingToStart : System.Web.UI.Page
{
    ...
    protected void  BindDetailsView()
    {
        ...
        if (Session["userType"].ToString().Trim() == "Super")
        {
            query = "Select *, (Select EmployeeName from EmployeeUserMaster where UserID=SCIH.SurveyEngineerID) SurveyEngineer, EUM.EmployeeName SalesPerson from ClientMaster CM, SurveyClientItemHead SCIH, EmployeeUserMaster EUM, ClientServiceMaster CSM where CM.ClientID=SCIH.ClientID and SCIH.ClientID=CSM.ClientID and CSM.EmployeeID=EUM.UserID and SCIH.StartDate is null order by SurveyID";
        ...
        {
            query = "Select *, (Select EmployeeName from EmployeeUserMaster where UserID=SCIH.SurveyEngineerID) SurveyEngineer, EUM.EmployeeName SalesPerson from ClientMaster CM, SurveyClientItemHead SCIH, EmployeeUserMaster EUM, ClientServiceMaster CSM where CM.ClientID=SCIH.ClientID and SCIH.ClientID=CSM.ClientID and CSM.EmployeeID=EUM.UserID and SCIH.StartDate is null and SCIH.SurveyEngineerID='" + Session["userID"] + "' Order By SurveyID";
        }
