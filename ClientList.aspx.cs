...
public partial class ClientList : System.Web.UI.Page
{
    ...
    protected void  BindDetailsView(string clientID)
    {
        ...
        {
            if (clientID.Trim().Length > 0)
                query = "Select *, (Select EmployeeName from EmployeeUserMaster where UserID=CSM.SurveyEngineerID) SurveyEngineer, EUM.EmployeeName from ClientMaster CM, ClientServiceMaster CSM, EmployeeUserMaster EUM where CM.ClientID=CSM.ClientID and CSM.EmployeeID=EUM.UserID and ltrim(rtrim(CSM.EmployeeID))='" + Session["userID"].ToString().Trim() + "' and CM.ClientID='"+clientID+"'";
            else
                query = "Select *, (Select EmployeeName from EmployeeUserMaster where UserID=CSM.SurveyEngineerID) SurveyEngineer, EUM.EmployeeName from ClientMaster CM, ClientServiceMaster CSM, EmployeeUserMaster EUM where CM.ClientID=CSM.ClientID and CSM.EmployeeID=EUM.UserID and ltrim(rtrim(CSM.EmployeeID))='" + Session["userID"].ToString().Trim() + "'";
        }
